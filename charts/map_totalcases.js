const drawChart = async () => {
  ...
  const countrytocontinent = await d3.json(
    "../../data/countrytocontinent.json"
  );
  const colorScale = d3
    .scaleLinear()
  ...
  continentSelect.on("change", () => {
    ...
    map
      ...
      .attr("opacity", (d) =>
        ...
        countrytocontinent[d.properties.name] === selectedContinent
          ? 1
          : 0.1
      );
  });
  ...
  d3.select("#country-filter").on("input", () => {
    ...
    map
      .selectAll("path")
      .attr("opacity", (d) =>
        filterValue.some((country) =>
          d.properties.name.toLowerCase().includes(country.trim())
        )
          ? 1
          : 0.4
      );
  ...
  const filteredList = coviddata.filter((obj) => {
    ...
    return !Object.keys(obj).some(
      (key) => key === "Entity" && dataremoved.includes(obj[key])
    );
  });
  ...
  deathFilter.on("change", () => {
    ...
    if (selectedOption === "top10death") {
      ...
      map
        ...
        .attr("opacity", (d) =>
          filteredData.find((b) => b.Entity === d.properties.name) ? 1 : 0.2
        );
      ...
      map.selectAll("path").attr("opacity", (d) => selectedOption === "all");
    }
  });
  ...
  map
    ...
    .attr("fill", (d) =>
      colorScale(
        coviddata.find((b) => b.Entity === d.properties.name)?.total_cases || 0
      )
    )
    ...
    .on("click", (a, b) => {
      ...
      window.open("/cases_details.html", "_blank");
    })
    ...
    .on("mouseout", function (e, d) {
      const filteredData = coviddata.filter((item) =>
        filterValue.some((country) =>
          item.Entity.toLowerCase().includes(country.trim())
        )
      );
      const color =
        filteredData.length === 0
          ? "#f0eee5"
          : colorScale(filteredData[0].total_cases / 30);
      d3.select(this)
        .transition()
        .duration(100)
        .attr("fill", (d) =>
          colorScale(
            coviddata.find((b) => b.Entity === d.properties.name)
              ?.total_cases || 0
          )
        );
      d3.select(".tooltip").style("display", "none");
