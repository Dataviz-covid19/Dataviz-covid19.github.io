const drawChart = async () => {
  ...
  const filteredList = coviddata.filter((obj) => {
    ...
    return !Object.keys(obj).some(
      (key) => key === "location" && dataremoved.includes(obj[key])
    );
  });
  ...
  deathFilter.on("change", () => {
    ...
    if (selectedOption === "top10vaccine") {
      ...
      map
        ...
        .attr("opacity", (d) =>
          filteredData.find((b) => b.location === d.properties.name) ? 1 : 0.2
        );
    } else {
    ...
    .on("mousemove", function (e, d) {
      ...
      if (data) {
        let tooltip = d3
          ...
          .style("opacity", 1)
          .style("display", "block");
        const formattedTotalCases = data.total_vaccinations
          ? data.total_vaccinations.toLocaleString()
          : "NULL";
        d3.select(".tooltip")
          .html(
      ...
      d3.select(this)
        ...
        .attr("fill", (d) => {
          const filteredData = coviddata.filter((item) =>
            filterValue.some((country) =>
              item.location.toLowerCase().includes(country.trim())
            )
          );
          const color =
            filteredData.length === 0
              ? "#f0eee5"
              : colorScale(filteredData[0].total_vaccinations / 30);
          const data = coviddata.find((b) => b.location === d.properties.name);
          if (data && data.total_vaccinations != null) {
            return colorScale(data.total_vaccinations);
          } else {
