function drawChart_a1_v2() {
  ...
  let margin = { top: 50, right: 30, bottom: 100, left: 100 };
  let width = 400;
  let height = 400;
  ...
  d3.csv("../../data/data_barchart_wafflechart_piechart.csv").then(function (
    data
  ) {
    ...
    let sum = d3.sum(other, function (d) {
      return d.Total_Deaths;
    });

    // Use top20 to create the chart
    xScale.domain(
      top20.map(function (d) {
        return d.Country;
      })
    );
    yScale.domain([
    ...
    g.append("text")
      ...
      .text("Comparing Total Deaths among Top 20 countries using Bar chart");
    ...
    d3.select("#other").html(`<h2>Others</h2><div> Count: ${sum}</div>`);
  });
  ...
  function onMouseOver(d, i) {
    ...
    d3.select(this)
      .transition() // added animnation here
      .duration(500)
      .attr("width", xScale.bandwidth() + 5)
      .attr("y", function (d) {
        return yScale(d.Total_Deaths) - 10;
      })
      .attr("height", function (d) {
        return height - yScale(d.Total_Deaths) + 10;
      });
  }
